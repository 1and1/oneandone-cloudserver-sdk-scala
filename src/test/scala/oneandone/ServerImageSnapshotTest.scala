...
class ServerImageSnapshotTest extends FunSuite with BeforeAndAfterAll {
  ...
  test("Clone server") {
    Server.waitStatusAndPercentage(fixedServer.id, ServerState.POWERED_ON)
    serverClone = Server.clone(fixedServer.id, CloneServerRequest(name = "cloned Server"))
    Server.waitStatus(serverClone.id, ServerState.POWERED_ON)
  }
  ...
  test("Get Server DVD") {
    var dvd = Server.getDvd(fixedServer.id)
    assert(dvd != null)
  }
  ...
  test("Get Server image ") {
    var instance = Server.getImage(fixedServer.id)
  ...
  test("Reinstall Server Image") {
    ...
    var image   = Server.reinstallImage(fixedServer.id, request)
    Server.waitStatus(fixedServer.id, ServerState.POWERED_ON)
